...
const steps = ["Enviament", "Pagament", "Resum"];
...
const initialForm = {
    name: "", email: "", address: "", city: "", zip: "", country: "ES",
    cardName: "", cardNumber: "", expiry: "", cvv: "",
};

const cartItems = [
    { id: 1, name: "Samarreta Premium", qty: 2, price: 29.99 },
    { id: 2, name: "Gorra Clàssica", qty: 1, price: 19.99 },
];
...
export default function Checkout() {
    ...
    const validateStep = () => {
        const errs = {};
        if (step === 0) {
            if (!form.name) errs.name = "Nom obligatori";
            if (!form.email || !/\S+@\S+\.\S+/.test(form.email)) errs.email = "Email invàlid";
            if (!form.address) errs.address = "Adreça obligatòria";
            if (!form.city) errs.city = "Ciutat obligatòria";
            if (!form.zip) errs.zip = "Codi postal obligatori";
        }
        if (step === 1) {
            if (!form.cardName) errs.cardName = "Nom obligatori";
            if (form.cardNumber.replace(/\s/g, "").length !== 16) errs.cardNumber = "Número invàlid";
            if (!form.expiry || form.expiry.length < 5) errs.expiry = "Data invàlida";
            if (form.cvv.length !== 3) errs.cvv = "CVV invàlid";
        }
        setErrors(errs);
    ...
    if (done) return <Success name={form.name} onBack={() => navigate("/dashboard")} />;
                ...
                <div style={styles.body}>
                    <div style={styles.formPanel}>
                        ...
                        <div style={styles.btnRow}>
                            {step > 0 && (
                                <button style={styles.btnSecondary} onClick={prev}>← Atrás</button>
                            )}
                            {step < 2 && (
                                <button style={styles.btnPrimary} onClick={next}>Continuar →</button>
                            )}
                            {step === 2 && (
                                <button
...
function PaymentStep({ form, set, errors }) {
    return (
        <div>
            ...
            <p style={{ fontSize: 12, color: "#888", marginTop: 12 }}>🔒 Pago simulado. No se procesan datos reales.</p>
        </div>
    );
}
    ...
    return (
        <div>
            ...
            <div style={styles.summarySection}>
                ...
                <p style={styles.summaryText}>{form.name} · {form.email}</p>
                <p style={styles.summaryText}>{form.address}, {form.city} {form.zip}</p>
            </div>
            <div style={styles.summarySection}>
...
function OrderSummary() {
    return (
        <div style={styles.summaryBox}>
            ...
            <div style={styles.summaryRow}><span>Envío</span><span>{shipping.toFixed(2)} €</span></div>
            <div style={styles.divider} />
    ...
    return (
        <div style={{ minHeight: "100vh", background: "#f9fafb", display: "flex", justifyContent: "center", alignItems: "center" }}>
            <div style={styles.successBox}>
                ...
                <h2 style={{ margin: "16px 0 8px", color: "#111827" }}>¡Gracias, {name.split(" ")[0]}!</h2>
                <p style={{ color: "#666", textAlign: "center" }}>
                    Tu pedido ha sido confirmado correctamente.<br />Recibirás un correo de confirmación pronto.
                </p>
...
const styles = {
    page: { minHeight: "100vh", background: "#f9fafb", fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', display: "flex", justifyContent: "center", padding: "40px 16px" },
    ...
    label: { fontSize: 12, fontWeight: 600, color: "#555", textTransform: "uppercase", letterSpacing: .5 },
    ...
    cardChip: { fontSize: 18, letterSpacing: 2, marginBottom: 20, opacity: .6 },
    cardNum: { fontSize: 18, letterSpacing: 4, fontFamily: "monospace", marginBottom: 20 },
    cardMeta: { display: "flex", justifyContent: "space-between", fontSize: 13, opacity: .8, textTransform: "uppercase", letterSpacing: 1 },
    ...
    summarySubtitle: { fontSize: 12, fontWeight: 700, color: "#111827", textTransform: "uppercase", letterSpacing: .5, marginBottom: 6 },
